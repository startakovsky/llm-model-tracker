---
model: Qwen: Qwen3 30B A3B Thinking 2507
organization: qwen
license: Apache 2.0
release_date: 2026-07-01
last_updated: 2026-07-20
openrouter_id: qwen/qwen3-30b-a3b-thinking-2507
category: Lightweight
generated: stub-from-csv
---

# Qwen: Qwen3 30B A3B Thinking 2507

Open-weight model from **qwen**. Weights on HuggingFace.

## Quick Facts
- **OpenRouter ID:** `qwen/qwen3-30b-a3b-thinking-2507`
- **Context length:** 131K
- **License:** Apache 2.0
- **Category:** Lightweight
- **Quality score:** 64/100
- **Notes:** Qwen3 30B thinking July 2026

## API Providers
| Provider | Prompt $/M | Completion $/M | Context |
|---|---|---|---|
| OpenRouter (qwen/qwen3-30b-a3b-thinking-2507) | $0.1300 | $1.5600 | 131K |

## Self-Hosting
Open weights make this self-hostable. Confirm exact parameter count and architecture on the HuggingFace model card before speccing hardware. See sibling detail files in this tracker for quantization guidance on comparable models.

## Quality Assessment
Quality score 64/100 — capable mid-tier. Qwen3 30B thinking July 2026

> This is an auto-generated stub derived from `models.csv` to keep README links valid. Enrich with architecture, GGUF quants, and benchmarks as the model gains traction.
